...
#define object_pat(object, type, idx) (type*)(object->data + idx * sizeof(type))
#define object_val(object, type) *(type*)object->data
#define object_vat(object, type, idx) *(type*)(object->data + idx * sizeof(type))
...
void object_list_append(Object* list, Object* olist)
{
    ...
    memcpy(list->data + list->count * list->elemSize, olist->data, olist->count * list->elemSize);
...
void object_list_prepend(Object* list, Object* olist)
{
    ...
    memcpy(newbuf + olist->count * olist->elemSize, list->data, list->count * list->elemSize);
    free(list->data);
    list->data = newbuf;
...
{
    Object* retval = object_list_create(list->data + list->count - 1, 1, list->elemSize);
    list->count--;
    object_list_to_single(retval);
    return retval;
}

void object_list_push_front_value(Object* list, void* value)
{
    void* newbuf = calloc(list->capacity + 1, list->elemSize);
    list->count ++;
    list->capacity ++;
...
void object_list_push_back_value(Object* list, void* value)
{
...
Object* object_list_split(Object* list, void* value, size_t vcount)
{
    ...
    for (size_t i = 0; i < list->count; i ++)
    {
        ...
        {
            Object* blob = object_list_create(cursor, (current - cursor) / list->elemSize, list->elemSize);
            object_list_push_back_value(retval, blob);
    ...
    if (cursor < endptr)
    {
        Object* blob = object_list_create(cursor, (endptr - cursor) / list->elemSize, list->elemSize);
        object_list_push_back_value(retval, blob);
    }
...
#define object_create_string(string) object_list_create(string, strlen(string), 1)
...
int main(int argc, char** argv)
{
    ...
    {
        Object* current = object_vat(split, Object*, i);
        printf("Garble my warble!! %.*s\n", (int)current->count, object_ptr(current, char));
    }
    for (size_t i = 0; i < split2->count; i ++)
    {
        Object* current = object_vat(split2, Object*, i);
        printf("two!! %.*s\n", (int)current->count, object_ptr(current, char));
    }
